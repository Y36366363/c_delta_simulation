...
| Can a nonregular Huber/MAD profile be identified before local inference? | Studentization assumes a regular fitted reference and cannot repair mode-selection or vanishing-density boundaries. | A continuous shared-sign weak-null path recovered nominal behavior near radial log-SD `.40`; naive permutation remained `.157` while studentized rejection was `.030`. Largest central spacing/IQR tracked the failure. A provisional `.50` gate rejected the `.03-.20` path, retained `.997` of regular `t_5` and profile partial-null samples, but retained only `.797` under strong skew. | Treat `.50` as a warning sensitivity only; test sample-size scaling and alternative density/stability diagnostics before fixing a rule. |
