...
- Moderate and strong skew move the power crossover to `.131` and `.043`.
  Relative profile advantage still declines with dyadic weight, but individual
  profile, Mantel, and raw `c_delta_star` slopes can have different signs.
...
| Where does profile-versus-Mantel power change under mixed signals? | Real applications may contain both node salience and dyadic geometry. | Symmetric and skew derivatives are now explicit. Skew crossovers moved to `.131/.043`; total Huber movement can reverse individual slopes, but relative profile advantage still declines. | Test additional skew families only if they match the application; do not promote generator-specific crossover values to a selection rule. |
