...
| Can a nonregular Huber/MAD profile be identified before local inference? | Studentization assumes a regular fitted reference and cannot repair mode-selection or weak centre/scale identification. | The complete standardized nuisance Jacobian is now explicit and verified. Smaller `sqrt(n)*sigma_min(J)` correlated `.971` with rejection; the binding symmetric direction was `d*f(m)`. MAD endpoint density and Huber curvature added little first-order fit, while a skew benchmark restored the expected off-diagonal coupling. The residual family effect is therefore higher-order/nonlocal. | Keep the singular value and MAD-scale/IQR descriptive. Compare second moments/tails of the complete nuisance influence vector and numerically bound the second-order remainder before deciding whether an identification condition belongs in the theorem or only reporting guidance. |
