...
| Can a nonregular Huber/MAD profile be identified before local inference? | Studentization assumes a regular fitted reference and cannot repair mode-selection or weak centre identification. | Independent fine-grid and bridge simulations confirm the mechanism. Positive `f(0)=epsilon/2` alone did not calibrate finite samples; recovery approximately followed `n*epsilon^2`. Root-n bootstrap spread tracked the transition and never warned in eight external cells, but passing its exploratory threshold did not yield conditionally valid rejection. | Keep all three measures descriptive. Change the bridge family while matching `f(0)` and `n*f(0)^2`; only then decide whether quantitative identification belongs in theorem assumptions or reporting guidance. |
