...
| Can a nonregular Huber/MAD profile be identified before local inference? | Studentization assumes a regular fitted reference and cannot repair mode-selection or weak centre/scale identification. | Four bridge families matched on `f(0)` retained the main `n*f(0)^2` ordering, but a 500-dataset transition check found a modest residual family effect. Continuous spacing ordered cells, central density did not, and bootstrap location spread omitted MAD collapse. Small full-sample `MAD-scale/IQR` correlated `.726-.777` with rejection and a `.40` warning had low external cost, but none of these produced conditionally valid retained p-values. | Keep diagnostics descriptive. Numerically evaluate the complete nuisance Jacobian—centre density, MAD endpoint densities, and Huber score curvature—before deciding whether a quantitative identification condition belongs in the theorem or only reporting guidance. |
