...
| Can a nonregular Huber/MAD profile be identified before local inference? | Studentization assumes a regular fitted reference and cannot repair mode-selection or vanishing-density boundaries. | Across `n=40-640`, spacing detected gross separation but a fixed `.50` threshold was sample-size/skew sensitive. Central valley density described the structural boundary but warned on the `.20` path after rejection recovered. Root-n scaled bootstrap reference spread best tracked persistent failure: external-null medians were `1.03-1.39`, versus `2.53-5.22` on the severe `.10` path. | Keep assumption diagnostics and estimator-stability diagnostics separate. Independently calibrate the exploratory scaled-spread boundary `2` with at least 199 bootstrap draws before any promotion to a formal gate. |
