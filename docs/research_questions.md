...
| Is the subgroup-size effect only a permutation-resolution effect? | Larger `k` increases both signal strength and changes the permutation overlap layers. | Added lower-target calibrated alternatives at target correlations `0.35`, `0.55`, and `0.65`. Target `0.35` gives the clearest non-ceiling comparison; `0.55` and `0.65` are still near-saturated for normal. | Extend the target `0.35` setting to L1 and possibly rank-based variants. |
