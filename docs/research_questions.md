...
| Is the subgroup-size effect only a permutation-resolution effect? | Larger `k` increases both signal strength and changes the permutation overlap layers. | Current results show stronger detection for larger matched subgroups, but this cannot yet be attributed only to `1 / choose(n, k)`. | Build calibrated alternatives where `k = 1, 2, 3` have comparable mean correlation or normalized `c_delta`. |
